...
3. **`.env` 파일 설정**:  
   프로젝트 루트에 `.env` 파일을 생성하고, 아래와 같이 EduKisa 로그인 정보를 입력합니다:
...
- **`SafeEdu` Class**:
   - 전체 자동화 프로세스를 관리
   - 로그인, 학습 콘텐츠 탐색, 시험 제출, 브라우저 종료 등의 기능

- **`log_in` function**:
   - `.env` 파일에 정의된 사용자 정보를 통해 플랫폼 로그인
   
- **`find_next_study_button` function**:
   - "학습하기" 또는 "시험보기" 버튼을 찾아 클릭, 학습과 시험 자동으로 진행

- **`submit_test` function**:
   - 자동으로 답안을 선택, 시험 제출 버튼 클릭

- **`get_network_log` function**:
   - 네트워크 로그를 가져와서 시험 응답 데이터 추출
